...
class TradeViewsTestCase(unittest.TestCase):
    ...
    # @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b103')
    # def test_require_order_rcc(self):
    #     """
    #     Return the RCC of the order within a JSON object
    #     """
    #     add_database_to_settings('test_kc_ems')
    #     call_command('loaddata', 'orders.yaml', database='test_kc_ems')
    #     response = self.client.get(reverse('trade:require_order_rcc'), {'order_id': '55d1feb60008099bd4151fa3'})
    #     self.assertEqual(response.status_code, 302)
    #     self.client.login(username='member3', password='admin')
    #     response = self.client.get(reverse('trade:require_order_rcc'), {'order_id': '55d1feb60008099bd4151fa3'})
    #     self.assertEqual(response.status_code, 200)
    ...
    #     self.assertEqual(response['rcc'], order.rcc)
    #     self.assertEqual(order.status, Order.PROCESSING)
    #
    #     order = Order.objects.get(pk='55d1feb60008099bd4151fa3')
    #     self.assertEqual(order.delivery_option.company, get_service_instance())
    #     self.assertEqual(Order.objects.using('test_kc_ems').filter(pk='55d1feb60008099bd4151fa3').all().count(), 0)
    #     self.assertEqual(Package.objects.using('test_kc_ems').filter(order=order).all().count(), 0)
    #
    # @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b103')
    # def test_require_order_rcc_after_delay_allowed(self):
    #     """
    #     Return JSON object {'error': 'Timeout'} when requiring RCC
    #     of an Order after the TIME_LEFT_TO_COMMIT_TO_SELF_DELIVERY defined in ikwen_kakocase.kakocase.models
    #     """
    #     order = Order.objects.get(pk='55d1feb60008099bd4151fa1')
    #     max_delay = TIME_LEFT_TO_COMMIT_TO_SELF_DELIVERY + 2
    #     order.created_on = datetime.now() + timedelta(seconds=max_delay)
    #     order.save()
    #     self.client.login(username='member3', password='admin')
    #     response = self.client.get(reverse('trade:require_order_rcc'), {'order_id': '55d1feb60008099bd4151fa1'})
    #     self.assertEqual(response.status_code, 403)
    #     response = json.loads(response.content)
    #     self.assertEqual(response['error'], 'Timeout')

    @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b103')
    def test_LateDeliveryList(self):
        """
        Page must return HTTP 200 status. Access is limited to authorized collaborators only
        """
        response = self.client.get(reverse('trade:late_delivery_list'))
        self.assertEqual(response.status_code, 302)
        self.client.login(username='member3', password='admin')
        response = self.client.get(reverse('trade:late_delivery_list'))
        self.assertEqual(response.status_code, 200)
        late_deliveries = response.context['late_deliveries']
        self.assertGreaterEqual(late_deliveries.count(), 1)

    @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b103')
    def test_LateDeliveryList_json_format(self):
        """
        Requesting late_deliveries list with GET parameter format=json&q=searchTerm should.
        Result must return as a list of JSON objects
        """
        self.client.login(username='member3', password='admin')
        response = self.client.get(reverse('trade:late_delivery_list'),
                                   {'format': 'json', 'q': 'kam', 'start': 0, 'length': 24})
        late_deliveries = json.loads(response.content)
        self.assertEqual(len(late_deliveries), 1)
        self.assertEqual(late_deliveries[0]['id'], '57d1feb60bd4100809951de1')

    @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b103')
    def test_BrokenProductList(self):
        """
        Page must return HTTP 200 status. Access is limited to authorized collaborators only
        """
        response = self.client.get(reverse('trade:broken_product_list'))
        self.assertEqual(response.status_code, 302)
        self.client.login(username='member3', password='admin')
        response = self.client.get(reverse('trade:broken_product_list'))
        self.assertEqual(response.status_code, 200)
        broken_products = response.context['broken_products']
        self.assertGreaterEqual(broken_products.count(), 1)

    @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b103')
    def test_BrokenProductList_json_format(self):
        """
        Requesting broken_products list with GET parameter format=json&q=searchTerm should.
        Result must return as a list of JSON objects
        """
        self.client.login(username='member3', password='admin')
        response = self.client.get(reverse('trade:broken_product_list'),
                                   {'format': 'json', 'q': 'kam', 'start': 0, 'length': 24})
        broken_products = json.loads(response.content)
        self.assertEqual(len(broken_products), 1)
        self.assertEqual(broken_products[0]['id'], '5760bd4100809951ded1feb1')
