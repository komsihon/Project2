...
class Order(Model):
    ...
    def split_into_packages(self):
        ...
        for entry in self.entries:
            ...
            retail_cost = entry.count * entry.product.retail_price
            if service == provider:
                provider_revenue = entry.count * entry.product.retail_price
                retailer_revenue = 0
            else:
                provider_revenue = entry.count * entry.product.wholesale_price
                retailer_revenue = entry.count * (entry.product.retail_price - entry.product.wholesale_price)
